...
export async function analyzeScript(script: string, referenceImages?: string[], customApiKey?: string, signal?: AbortSignal): Promise<AnalysisResult> {
  ...
  const contentParts: any[] = [
    { text: `
      ...
      4. 资产一致性关联：你必须保证生成的【角色/场景/道具表】中的名称与细节，能够精确地体现在分镜的视觉描述(visualDescription)与(videoPrompt)中。例如：如果在资产表中有一个角色叫“李雷”，当李雷出现在分镜中时，你必须在 visualDescription 里直接带入他的设定信息(如：李雷，穿着黑色夹克...)，保证生图提示词能够直接体现这些资产特征。
